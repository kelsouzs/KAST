...
def ensure_reproducibility(seed=42):
    ...
    # Note: TF_DETERMINISTIC_OPS and enable_op_determinism disabled for CPU-only systems
    # These are GPU-related features that can cause issues when CUDA is not available
    ...
    while attempts < 3:
        try:
            ...
            if len(filename) > 50:
                attempts += 1
                if attempts < 3:
                    print(f"⚠️ Filename too long. Use less than 50 characters. ({3 - attempts} attempts left)")
                    print("-"*70)
                else:
                    print("❌ Too many invalid attempts. Using default name...")
                    return "5_1_predictions"
                continue
    ...
    while attempts < 3:
        try:
            ...
            if choice == '1':
                ...
                while sub_attempts < 3:
                    try:
                        ...
                        if 0.0 <= cutoff <= 1.0:
                            ...
                            sub_attempts += 1
                            if sub_attempts < 3:
                                print(f"⚠️ Please enter a value between 0.0 and 1.0 ({3 - sub_attempts} attempts left)")
                            else:
                        ...
                        sub_attempts += 1
                        if sub_attempts < 3:
                            print(f"⚠️ Please enter a valid number ({3 - sub_attempts} attempts left)")
                        else:
                            print("❌ Too many invalid attempts. Returning to menu...")
                            return None
                return None
                ...
                while sub_attempts < 3:
                    try:
                        ...
                        if 1 <= top_n <= stats['total']:
                            ...
                            sub_attempts += 1
                            if sub_attempts < 3:
                                print(f"⚠️ Please enter a number between 1 and {stats['total']} ({3 - sub_attempts} attempts left)")
                            else:
                                print("❌ Too many invalid attempts. Returning to menu...")
                                return None
                    except ValueError:
                        sub_attempts += 1
                        if sub_attempts < 3:
                            print(f"⚠️ Please enter a valid integer ({3 - sub_attempts} attempts left)")
                        else:
                            print("❌ Too many invalid attempts. Returning to menu...")
                            return None
                return None
                ...
                attempts += 1
                if attempts < 3:
                    print(f"⚠️ Invalid option. Please select 1-5. ({3 - attempts} attempts left)")
                else:
                    print("❌ Too many invalid attempts. Returning to menu...")
                    return None
