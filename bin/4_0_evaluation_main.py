...
def clean_previous_evaluation_files(results_dir):
    ...
    if not existing_files:
        return
    print(f"\n\033[93mWARNING: Files from a previous evaluation were found.\033[0m")
    attempts = 0
    while attempts < 3:
        choice = input("Do you want to overwrite them for a new evaluation? (y/n): ").strip().lower()
        if choice in ['y', 'yes']:
            ...
            attempts += 1
            if attempts < 3:
                print(f"❌ Invalid input. Please enter 'y' or 'n'. ({3 - attempts} attempts left)")
            else:
                print("❌ Too many invalid attempts. Using default (no overwrite).")
                return
